...
The FastAPI app, its lifespan, and all routes live here (not in the orders kernel).
The composition root in container.py wires the ports this app uses; the lifespan builds
the Temporal client via appkit (data-converter contract baked in) and wraps the domain
TemporalService around it. Run with: uvicorn main:app
"""
...
from appkit import Database, build_temporal_client
