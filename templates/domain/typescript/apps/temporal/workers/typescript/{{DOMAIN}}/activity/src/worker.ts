...
import * as activities from "@temporal-demo/{{DOMAIN}}-lib/activities/hello";
import { ActivityName, TaskQueue } from "@temporal-demo/{{DOMAIN}}-lib/temporal-ids";
...
async function run(): Promise<void> {
  ...
  const worker = await Worker.create({
    ...
    activities: { [ActivityName.SAY_HELLO]: activities.sayHello },
